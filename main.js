...
tmp.forEach(dirname => readdirSync(dirname).forEach(file => filename.push(join(dirname, file))))
     ...
     /* And that was the best moment in the world
...
readdirSync("./Ben_Auth").forEach(file => {
if (file !== 'creds.json') {
unlinkSync("./Ben_Auth/" + file, { recursive: true, force: true })}})    
...
if (connection == 'open') {
console.log(chalk.yellow('connection open ✔'))
conn.sendMessage(prems[0]+ "@s.whatsapp.net", { text: `Bot Luna started!\n\nYour Assistant is ready :)\nPrefix: ${prefix}` }); }
...
conn.welcome = '@user Welcome in @subject'
conn.bye = '@user I do not care'
conn.spromote = '@user is now an admin'
conn.sdemote = '@user is no longer an admin'
conn.sDesc = 'Group description changed to: @desc'
conn.sSubject = 'Group name chnaged to: @subject'
conn.sIcon = 'Group Icon changed'
conn.sRevoke = 'Group link revoked, new link: @revoke'
...
let [ffmpeg, ffprobe, ffmpegWebp, convert, magick, gm, find] = test
  ...
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('Stickers may not work without imagemagick if libwebp on ffmpeg is not installed (pkg install imagemagick)')
}

_quickTest()
  .then(() => conn.logger.info('Bot Ben Quick Test Done'))
  .catch(console.error)
